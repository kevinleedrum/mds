...
const McInput = class {
  ...
  render() {
    return (h(Host, null, this.label && (h("label", { htmlFor: this.inputId, class: "block text-secondary font-bold subtitle4 mb-10 uppercase" }, this.label, this.required && h("span", { class: "text-status-error" }, "*"))), this.type !== 'textarea' ? (h("div", { class: "flex items-center relative" }, this.leftIcon && h("i", { class: `leftIcon ${this.leftIcon}` }), this.type !== 'file' ? (h("input", { id: this.inputId, class: this.makeInputClasses, type: this.type, name: this.name, value: this.value, placeholder: this.placeholder, disabled: this.disabled ? true : false, readonly: this.readonly ? true : false, "aria-label": this.elAriaLabel, onFocus: this.handleInputFocus.bind(this), onBlur: this.handleInputBlur.bind(this), ref: el => (this.elemInput = el) })) : (h("div", { class: "w-full" }, h("input", { type: "text", class: `w-full ${this.makeInputClasses}`, onClick: this.triggerFileSelection.bind(this), ref: el => (this.elemFileUploadNameHolder = el), placeholder: this.placeholder, disabled: this.disabled ? true : false, readonly: true }), h("input", { type: "file", ref: el => (this.elemFileInput = el), onChange: this.handleFileUploadChange.bind(this), name: this.name }))), this.type === 'search' && (h("mc-button", { ref: el => (this.btnSearch = el), class: "hidden", small: true }, this.searchLabel)), this.type === 'file' && (h("mc-button", { onClick: this.triggerFileSelection.bind(this), disabled: this.disabled ? true : false, small: true }, "Choose File")))) : (h("textarea", { id: this.inputId, class: this.makeInputClasses, name: this.name, value: this.value, placeholder: this.placeholder, disabled: this.disabled ? true : false, readonly: this.readonly ? true : false, "aria-label": this.elAriaLabel, ref: el => (this.elemInput = el) })), this.instructions && !this.error && (h("section", { class: "text-secondary caption1 mt-10" }, this.instructions)), this.error && this.errorMsg && (h("section", { class: "flex caption1 mt-10 text-status-error items-center gap-6" }, h("i", { class: "ph-warning" }), this.errorMsg))));
  }
};
