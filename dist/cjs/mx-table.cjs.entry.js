  ...
  render() {
    ...
    if (this.checkable) {
      multiRowActionUI =
        ...
        index.h("span", { class: !this.checkedRowIds.length ? 'hidden' : null }, index.h("mx-button", { ref: el => (this.actionMenuButton = el), "btn-type": "text", dropdown: true }, index.h("span", { class: "h-full flex items-center" }, index.h("i", { class: "ph-gear text-1" }))), index.h("mx-menu", { "data-testid": "multi-action-menu", ref: el => (this.actionMenu = el) }, this.multiRowActions.map(action => (index.h("mx-menu-item", Object.assign({}, action), action.value))))));
    }
